...
const [imageUri, setImageUri] = useState<any>();
 ...
 if(value==='View'){
  ...
  const optionsr = [
    {label: 'Add Payment', value: 'addPayment', color: 'black', select: false},
...
const { bg: pillBg, fg: pillFg } =
  STATUS_PALETTE[data?.status] || { bg: '#F0F0F0', fg: '#333' };
...
const fetchCustomerPayments = async () => {
  try {
    const response = await fetch(`https://api.reparv.in/sales/customers/payment/get/${data?.enquirerId}`, {
      method: 'GET',
      credentials: 'include',
      headers: {
...
const getCustomer=async()=>{
  ...
  try {
    fetch(`https://api.reparv.in/sales/customers/payment/get/${data.enquirerid}`, {
      method: 'GET',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
 ...
 const addCustomerPayment = async () => {
  ...
  try {
    const response = await fetch(
      `https://api.reparv.in/sales/customers/payment/add/${data?.enquirerid}`,
      {
        method: 'POST',
        credentials: 'include',
        body: formData,
...
>
  ...
  <View style={styles.centeredOverlay}>
    <View style={styles.optionPopup}>
      <Text style={[styles.popupTitle,{color:'black'}]}>Select Action</Text>
  ...
  <Text style={{fontSize: 12, marginBottom: 6}}>Payment Type</Text>
  <View
    style={{
      borderWidth: 1,
      borderColor: '#ccc',
...
>
    ...
    <View style={styles.enhancedModal}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.itemText}>Name: <Text style={styles.value}>{data?.fullname}</Text></Text>
          <Text style={styles.itemText}>Contact: <Text style={styles.value}>{data?.contact}</Text></Text>
        </View>
        {/* Property Info */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>🏠 Property Info</Text>
          <Text style={styles.itemText}>Property: <Text style={styles.value}>{data?.propertyName || 'N/A'}</Text></Text>
        </View>
     ...
     <View style={styles.section}>
          ...
          <Text style={styles.itemText}>
            Deal Amount: <Text style={[styles.value, { color: '#000' }]}>
              ₹{formatIndianAmount(data?.dealamount)}
            </Text>
          </Text>

          <Text style={styles.itemText}>
            Token Amount: <Text style={[styles.value, { color: 'green' }]}>
              ₹{formatIndianAmount(data?.tokenamount)}
            </Text>
          </Text>

          <Text style={styles.itemText}>
            Total Paid Amount With Token: <Text style={[styles.value, { color: 'green' }]}>
              ₹{formatIndianAmount(
                (parseFloat(data?.tokenamount) || 0) +
                customerPayments.reduce((sum, p) => sum + (parseFloat(p?.paymentAmount) || 0), 0)
              )}
            </Text>
          </Text>

          <Text style={styles.itemText}>
            Remaining Balance: <Text style={[styles.value, { color: 'red' }]}>
              ₹{formatIndianAmount(
  ...
  {customerPayments.length === 0 ? (
    ...
    customerPayments.map((payment, index) => (
      <View key={index} style={styles.paymentCard}>
        <View style={styles.paymentCardRow}>
          ...
          {payment?.paymentImage ? (
            <TouchableOpacity
              onPress={() =>
                setPreviewImage(`https://api.reparv.in${payment?.paymentImage}`)
              }>
              <Image
                source={{ uri: `https://api.reparv.in${payment?.paymentImage}` }}
      ...
      {/* Close Button */}
      <TouchableOpacity style={styles.closeBtn} onPress={() =>{ setModalVisible(false);setCustomerPayments([])}}>
