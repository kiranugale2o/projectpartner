...
const Community: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const [salesRes, territoryRes, onboardingRes, partnerRes] =
        await Promise.all([
      ...
      const [salesData, territoryData, onboardingData, partnerData] =
        await Promise.all([
          salesRes.json(),
          territoryRes.json(),
          onboardingRes.json(),
 ...
 const getPosts = async () => {
  ...
  const fetchFollowingPosts = async () => {
    try {
      const res = await fetch(
        `https://api.reparv.in/salesapp/user/${auth?.user?.id}/following-posts`,
      );
      const data = await res.json();
      setPost(data);
      console.log(data, 'ff');
    ...
    useCallback(() => {
      if (selectedTab === 'discussions') {
        const fetchFollowersAndFollowing = () => {
          fetch(
            `https://api.reparv.in/salesapp/user/${auth?.user?.id}/followers`,
          )
            .then(res => res.json())
            .then(setFollowers);
      ...
      <View style={styles.frame135}>
          ...
          >
            <Svg width={21} height={20} viewBox="0 0 21 20" fill="none">
              <Path
                ...
                fill={selectedTab === 'discussions' ? '#076300' : 'gray'}
              />
            </Svg>
          </View>
          <Text
          ...
          >
            <Svg width={21} height={20} viewBox="0 0 21 20" fill="none">
              <Path
                ...
                fill={selectedTab === 'follow' ? '#076300' : 'gray'}
              />
            </Svg>
          </View>
          <Text
        ...
        >
          ...
          >
            <BookA
              size={21}
              color={selectedTab === 'MyPost' ? '#076300' : 'gray'}
              strokeWidth={2}
            />
          </View>
          <Text
