...
console.log(auth?.user?.salespersonsid,"ffuuuuuuuuuffffff");

  const showDetails = async () => {
    try {
      const res = await fetch(`https://api.reparv.in/admin/salespersons/get/${auth?.user?.id}`, {
        method: 'GET',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
      });
      if (!res.ok) throw new Error('Failed to fetch details',res);
      const data = await res.json();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://api.reparv.in/admin/salespersons/edit/${auth?.user?.id}`, {
        method: 'PUT',
        credentials: 'include',
        body: formData,
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <TextInput style={styles.input} placeholder="Account Holder Name*" value={userData.accountholdername} placeholderTextColor={'gray'} onChangeText={(t) => setUserData({ ...userData, accountholdername: t })} />
        <TextInput style={styles.input} placeholder="Account Number*" keyboardType="number-pad" value={userData.accountnumber} placeholderTextColor={'gray'} onChangeText={(t) => /^\d{0,17}$/.test(t) && setUserData({ ...userData, accountnumber: t })} />
        <TextInput style={styles.input} placeholder="IFSC Code*" autoCapitalize="characters" value={userData.ifsc} placeholderTextColor={'gray'} onChangeText={(t) => /^[A-Z0-9]{0,11}$/.test(t) && setUserData({ ...userData, ifsc: t.toUpperCase() })} />
  ...
  >
    ...
    {citiesList.map((c, i) => (
      <Picker.Item key={i} label={c.city} value={c.city} />
        ...
        <TextInput style={styles.input} placeholder="Pin Code*" keyboardType="number-pad" value={userData.pincode?.toString() || ''}
